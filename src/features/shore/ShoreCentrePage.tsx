...
import { SHORE_FUNCTION_LABELS, type ShoreCase, type ShoreFunction } from '@/types'
      ...
      <Panel title={`Shore Assistance Cases (${shoreCases.length})`}>
        {shoreCases.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {shoreCases.map((c) => (
              <button key={c.id} onClick={() => setActiveCase(c)} className="w-full rounded-md border border-panel-border bg-panel-raised p-3 text-left transition-colors hover:border-info-500/40">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Pill tone="info">{c.status.replace(/_/g, ' ')}</Pill>
                  </div>
                </div>
      ...
      {activeCase && (
        <Modal title={`${activeCase.id} — ${activeCase.vesselName}`} onClose={() => setActiveCase(null)} wide>
          <div className="flex flex-col gap-3 text-sm">
            ...
            <Field label="Status" value={activeCase.status.replace(/_/g, ' ')} />
            ...
            <div>
              ...
              <textarea value={guidance} onChange={(e) => setGuidance(e.target.value)} rows={2} className="w-full rounded-md border border-hull-500/40 bg-hull-800 px-2.5 py-2 text-sm text-ink-100" />
            </div>
            <div className="flex flex-wrap gap-2 border-t border-panel-border pt-3">
              ...
              <Button size="sm" variant="ghost" onClick={() => updateShoreCase(activeCase.id, 'in_review')}>REVIEW EVIDENCE</Button>
              ...
              <Button size="sm" variant="secondary" onClick={() => updateShoreCase(activeCase.id, 'returned')}>RETURN TO VESSEL</Button>
    ...
    <Modal title="New Shore Assistance Request" onClose={onClose}>
      <div className="flex flex-col gap-3 text-sm">
        <div>
          ...
          <select value={fn} onChange={(e) => setFn(e.target.value as ShoreFunction)} className="w-full rounded-md border border-hull-500/40 bg-hull-800 px-2.5 py-2 text-sm text-ink-100">
            {FUNCTIONS.map((f) => (
        ...
        <div>
          ...
          <select value={priority} onChange={(e) => setPriority(e.target.value as typeof priority)} className="w-full rounded-md border border-hull-500/40 bg-hull-800 px-2.5 py-2 text-sm text-ink-100">
        ...
        <div>
          ...
          <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={2} className="w-full rounded-md border border-hull-500/40 bg-hull-800 px-2.5 py-2 text-sm text-ink-100" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-ink-400">Requested expertise</label>
          <input value={expertise} onChange={(e) => setExpertise(e.target.value)} className="w-full rounded-md border border-hull-500/40 bg-hull-800 px-2.5 py-2 text-sm text-ink-100" />
        </div>
        <Button
          variant="primary"
